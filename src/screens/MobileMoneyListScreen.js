/**
 * Mes comptes Mobile Money — liste uniquement.
 * Options par compte : Rendre par défaut, Supprimer (confirmation PIN).
 */
...
export function MobileMoneyListScreen({ navigation }) {
  const { token } = useApi();
  ...
  const confirmPin = async () => {
    ...
    try {
      if (action === 'delete') {
        ...
        setAccounts((prev) => prev.map((a) => ({ ...a, is_default: a.id === payload.id })));
      }
      closePinModal();
    } catch (e) {
  ...
  const onSetDefault = (acc) => {
    if (acc.is_default) return;
    Alert.alert('Définir par défaut', 'Confirmez avec votre PIN pour définir ce compte comme compte par défaut.', [
      { text: 'Annuler', style: 'cancel' },
      { text: 'Confirmer', onPress: () => askPin('set_default', { id: acc.id }) },
    ]);
  };
    ...
    <>
      ...
      <SafeAreaView style={styles.safe} edges={['top']}>
        <LinearGradient colors={[colors.primary, '#3d0f5c']} style={styles.header} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
          ...
          <Text style={styles.headerTitle}>Mes comptes Mobile Money</Text>
        ...
        >
          {loading ? (
            ...
            <>
              {accounts.length === 0 ? (
                <Text style={styles.emptyText}>Aucun compte Mobile Money enregistré.</Text>
              ) : (
                accounts.map((acc) => (
                  <View key={acc.id} style={styles.row}>
                    ...
                    <View style={styles.rowActions}>
                      {!acc.is_default && (
                        <TouchableOpacity style={styles.rowBtn} onPress={() => onSetDefault(acc)}>
                          <FontAwesome5 name="star" size={14} color={colors.secondary} />
                          <Text style={styles.rowBtnLabel}>Défaut</Text>
                        </TouchableOpacity>
                      )}
